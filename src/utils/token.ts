...
export async function getTokenBurned(
    ...
    if (assetId.__kind === 'Native') {
        ...
        if (tokenAccountsStorage.isV3) {
            result = await tokenAccountsStorage.asV3.get(account, assetId as v3.CurrencyId)
        } else if (tokenAccountsStorage.isV8) {
            result = await tokenAccountsStorage.asV8.get(account, assetId as v8.CurrencyId)
        }
    }
