...
import {EventHandlerContext, TOKEN_EVENT_TYPE} from "../types";
...
export async function handleTokenDeposited(ctx: EventHandlerContext, type: TOKEN_EVENT_TYPE) {
...
export async function handleTokenWithdrawn(ctx: EventHandlerContext, type: TOKEN_EVENT_TYPE) {
    const transactionHash = ctx.event.extrinsic?.hash
    if (!transactionHash) return
    let event
    if (type === TOKEN_EVENT_TYPE.Currencies) {
...
export async function handleTokenTransfer(ctx: EventHandlerContext, type: TOKEN_EVENT_TYPE) {
    let event
    if (type === TOKEN_EVENT_TYPE.Currencies) {
